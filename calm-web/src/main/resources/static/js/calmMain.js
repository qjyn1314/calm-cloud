...
layui.use(["element", "form", "layer", "okUtils", "okTab", "okLayer", "okContextMenu", "laydate", "okCookie"], function () {
    ...
    function setUserMessage(response) {
        ...
        let menuTree = JSON.stringify(user.menuTree);
    }

    let okTab = layui.okTab({
        // 菜单请求路径
        // url: "../static/data/navs.json",
        url: "../static/data/navs_dev.json",
        // url: okUtils.userMenuTree,
        // data: menuTree,
        // 允许同时选项卡的个数
        // openTabNum: 10,
    ...
    function noticeFun() {
        ...
        let contentText = "欢迎使用脚手架。";

        layer.open({
            type: 0, title: "系统公告", btn: "我知道啦", btnAlign: 'c', content: contentText,
            yes: function (index) {
                if (srcWidth > 800) {
