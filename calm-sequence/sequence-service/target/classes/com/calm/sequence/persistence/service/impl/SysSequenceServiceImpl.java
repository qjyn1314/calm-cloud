...
public class SysSequenceServiceImpl implements SysSequenceService {
    ...
    @Override
    public PageInfo<SysSequence> sequencePage(SysSequenceDto sysSequence) {
        LambdaQueryWrapper<SysSequence> lambdaedQuery = Wrappers.lambdaQuery(SysSequence.class);
        lambdaedQuery.eq(StrUtil.isNotBlank(sysSequence.getSequenceType()), SysSequence::getSequenceType, sysSequence.getSequenceType());
        lambdaedQuery.eq(Objects.nonNull(sysSequence.getCreateTime()), SysSequence::getCreateTime, sysSequence.getCreateTime());
        return PageHelper.startPage(sysSequence.getPageNum(), sysSequence.getPageSize())
                .doSelectPageInfo(() ->
                        sysSequenceMapper.selectList(lambdaedQuery)
                );
    }
}
